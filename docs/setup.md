...
```powershell
conda create -n fishrag python=3.11 -y
conda activate fishrag
...
python -m compileall apps/api packages tests
```
...
- 阶段 1 已完成数据库迁移验证；本地启动后请先确认 PostgreSQL/pgvector、Redis、OpenSearch 容器均为 healthy。
- 阶段 2 已接入文档上传落盘，默认文件会写入 `storage/uploads`，可通过 `FISHRAG_UPLOAD_DIR` 调整。
