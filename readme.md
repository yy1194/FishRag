...
本仓库将从 0 开始逐步搭建，优先完成一个可运行的 MVP，再逐步增强为企业级 RAG-Agent 平台。当前已完成基础工程骨架、环境变量模板、中间件编排文件、后端 API 入口、前端 Vite 骨架、Agent 规划工具雏形、RAG 文本切片雏形，以及 PostgreSQL/pgvector、Redis、OpenSearch 本地中间件配置。
...
- 提供 `write_todos` 工具，允许 Agent 创建、更新和维护任务清单。
...
- [ ] 创建 FastAPI 应用入口、健康检查和统一错误处理。
- [ ] 设计数据库表：用户、会话、消息、文档、chunk、任务、审批、记忆。
- [ ] 接入 PostgreSQL 迁移工具。
- [ ] 实现 JWT 登录鉴权和基础 RBAC。
- [ ] 实现结构化日志和请求追踪 ID。
...
- [ ] 实现文件上传接口。
- [ ] 实现 PDF、Word、TXT、Markdown、CSV 解析。
- [ ] 实现文本清洗、章节识别和 chunk 切分。
- [ ] 实现 embedding 接口适配层。
- [ ] 将 chunk 写入 pgvector。
- [ ] 将文本和元数据写入关键词索引。
- [ ] 实现文档状态流转：uploaded、processing、indexed、failed。
