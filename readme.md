...
本仓库将从 0 开始逐步搭建，优先完成一个可运行的 MVP，再逐步增强为企业级 RAG-Agent 平台。
...
| Embedding | bge-m3 / OpenAI-compatible embedding | 默认支持私有化模型，也保留云模型适配空间 |
| Reranker | bge-reranker / Cohere-compatible rerank | 对多路召回候选片段进行重排 |
...
- [ ] 确认 MVP 范围、用户角色和关键业务流程。
- [ ] 初始化 Python 后端工程结构。
- [ ] 初始化 React 前端工程结构。
- [ ] 配置 Docker Compose：PostgreSQL、Redis、OpenSearch。
- [ ] 配置代码规范：ruff、mypy、pytest、prettier、eslint。
- [ ] 建立 `.env.example` 和配置加载模块。
